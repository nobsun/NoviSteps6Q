...
solve :: Solver
solve = \ case
    (n,abss) -> case transpose abss of
        [as,bs] -> case maximumBy (comparing snd) (runLength (sort as)) of
            (a,p)   -> case maximumBy (comparing snd) (runLength (sort bs)) of
                (b,q)   -> if
                    | n == 3 -> "Yes"
                    | p == 1 -> bool "No" "Yes" (q == pred n || q + 1 == pred n && b `elem` as)
                    | q == 1 -> bool "No" "Yes" (p == pred n || p + 1 == pred n && a `elem` bs)
                    | p + q == pred n -> bool "No" "Yes" (a == b)
                    | otherwise -> "No"
        _       -> invalid

wrap :: Solver -> ([[I]] -> [[O]])
wrap f = \ case
    [n]:abss -> case f (n,abss) of
        rr -> [[rr]]
