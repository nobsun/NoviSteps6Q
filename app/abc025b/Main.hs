...
type Solver = (Int,Int,[(I,Int)]) -> [O]

solve :: Solver
solve = \ case
    (a,b,sds) -> iter 0 sds where
        iter c = \ case
            [] -> case compare c 0 of
                GT -> ["East", show c]
                EQ -> ["0"]
                LT -> ["West", show (abs c)]
            ("East", d):rs -> iter (c + d') rs
                where d' = min (max a d) b
            (_,d):rs       -> iter (c - d') rs
                where d' = min (max a d) b

wrap :: Solver -> ([[I]] -> [[O]])
wrap f = \ case
    [_,a,b]:sds -> case f (read a,read b, map (second read . toTuple) sds) of
        rr -> [rr]
