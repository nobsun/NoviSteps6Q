...
solve :: Solver
solve = \ case
    (h,w,css) -> case listArray ((1,1),(h,w)) (concat css) of
        a -> splitEvery w $ map (neighbors a) (indices a)

type Pos = (Int,Int)

neighbors :: Array Pos Char -> Pos -> Char
neighbors a = \ case
    (i,j)
        | a ! (i,j) == '#' -> '#'
        | otherwise -> intToDigit $ countif (('#'==) . (a !)) 
            $ filter (inRange (bounds a))
                [ (i+di,j+dj) | di <- [-1,0,1], dj <- [-1,0,1], (di,dj) /= (0,0) ]

wrap :: Solver -> ([[I]] -> [[O]])
wrap f = \ case
    hw:css -> case map read (words hw) of
        [h,w]  -> case f (h,w,css) of
            r -> r
        _ -> invalid
