...
solve :: Solver
solve = \ case
    (h,w,(si,sj),bss,x) -> case listArray ((1,1),(h,w)) (concat bss) of
        a -> case foldl' (phi a) (si,sj) x of
            (ei,ej) -> [ei,ej]
        where
            phi a (i,j) = \ case
                'L' | j == 1                -> (i,j)
...
wrap :: Solver -> ([[I]] -> [[O]])
wrap f = \ case
    hw:sij:cssx -> case map read (words hw) of
        [h,w]       -> case map read (words sij) of
            [si,sj]     -> case splitAt h cssx of 
                (bss, [x]) -> case f (h,w,(si,sj),bss,x) of
                    r -> [r]
                _ -> invalid
            _ -> invalid
        _ -> invalid
