...
solve = \ case
    (n,xxs) -> iter a0 xxs
        where
            a0 = IM.fromList (map (,0::Int) [1 .. n])
            iter a = \ case
                []   -> []
                0:xs -> case minimumBy (comparing snd) $ IM.assocs a of
                    (k,_) -> k : iter (IM.adjust succ k a) xs
                x:xs -> x : iter (IM.adjust succ x a) xs

wrap :: Solver -> ([[I]] -> [[O]])
wrap f = \ case
    [n,_]:xs:_ -> case f (n,xs) of
        rr -> singleton rr
