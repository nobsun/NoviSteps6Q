...
solve :: Solver
solve = \ case
    (p,q,r,s,as) -> case splitAt (pred p) as of
        (bs,rs) -> case splitAt (succ q - p) rs of
            (cs,ss) -> case splitAt (pred r - q) ss of 
                (ds,ts) -> case splitAt (succ s - r) ts of
                    (es,us) -> bs ++ es ++ ds ++ cs ++ us

wrap :: Solver -> ([[I]] -> [[O]])
wrap f = \ case
    [_,p,q,r,s]:as:_ -> case f (p,q,r,s,as) of
        rr -> [rr]
