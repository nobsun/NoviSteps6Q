...
solve :: Solver
solve = \ case
    (n,as,bs) -> case unzip $ catMaybes $ zipWith f as bs of
        (cs,ds) -> trace (show (cs,ds)) $ case n - length cs of
            a       -> case IS.size $ IS.fromList cs `IS.intersection` IS.fromList ds of
                b       -> [a,b]
    where
        f x y = bool Nothing (Just (x,y)) (x /= y)
