...
solve :: Solver
solve = \ case
    (ss,cs) -> case (zip [1::Int ..] ss, zip [1..] cs) of
        (iss,jcs) -> [ fst $ minimumBy (phi is) jcs | is <- iss ]
    where
        phi (_,p) = comparing dist
            where
                dist (_,q) = manhattan p q
...
wrap :: Solver -> ([[I]] -> [[O]])
wrap f = \ case
    [n,_]:pss -> case f (splitAt n (map toTuple pss)) of
        r -> map (:[]) r
