...
solve :: Solver
solve = \ case
    (h,w,mat) -> case listArray ((1,1),(h,w)) (concat mat) of
        aa  -> case [ (i1,i2) | i1 <- [1 .. h], i2 <- [succ i1 .. h] ] of
            ips -> case [ (j1,j2) | j1 <- [1 .. w], j2 <- [succ j1 .. w] ] of
                jps -> case [ (i1,i2,j1,j2) | (i1,i2) <- ips, (j1,j2) <- jps ] of
                    ijps -> bool "No" "Yes" $ all (check aa) ijps

check :: Array (Int,Int) Int -> (Int,Int,Int,Int) -> Bool
check aa (i1,i2,j1,j2) = aa ! (i1,j1) + aa ! (i2,j2) <= aa ! (i2,j1) + aa ! (i1,j2)

wrap :: Solver -> ([[I]] -> [[O]])
wrap f = \ case
    [h,w]:mat -> case f (h,w,mat) of
        rr -> [[rr]]
