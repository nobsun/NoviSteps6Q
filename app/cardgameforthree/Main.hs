...
solve :: Solver
solve = \ case
    asbscs -> iter 'A' asbscs where
        iter = \ case 
            'A' -> \ case
                ([],_,_)     -> 'A'
                (a:as,bs,cs) -> iter (toUpper a) (as,bs,cs)
            'B' -> \ case
                (_,[],_)     -> 'B'
                (as,b:bs,cs) -> iter (toUpper b) (as,bs,cs)
            _   -> \ case
                (_,_,[])     -> 'C'
                (as,bs,c:cs) -> iter (toUpper c) (as,bs,cs)

wrap :: Solver -> ([[I]] -> [[O]])
wrap f = \ case
    as:bs:cs:_ -> case f (as,bs,cs) of
        r -> [[r]]
