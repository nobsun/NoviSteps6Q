...
solve :: Solver
solve = \ case
    ss -> case dropWhile p ss of
        u:us -> case elemIndices 'o' u of
            [c,d]    -> abs (c - d)
            [c]      -> case spanCount p us of
                (b,v:_) -> case elemIndices 'o' v of
                    [d]  -> succ b + abs (c - d)
                    _    -> invalid
                _ -> invalid
            _ -> invalid
...
{- |
>>> breakCount even [3,1,4,1,5,9]
(2,[4,1,5,9])
-}
breakCount :: (a -> Bool) -> [a] -> (Int,[a])
breakCount p = \ case
    [] -> (0,[])
    aas@(a:as)
        | p a       -> (0,aas)
        | otherwise -> case breakCount p as of
            (c,bs)      -> (succ c, bs)
