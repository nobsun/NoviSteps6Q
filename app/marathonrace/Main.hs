...
solve :: Solver
solve = \ case
    as -> case zip [1::Int ..] as of
        ias -> case groupBy ((==) `on` snd) $ sortBy (comparing snd) ias of
            iass -> case map length iass of
                rs -> case scanl (+) 1 rs of
                    ss -> map snd $ sort $ concat $ zipWith phi iass ss
    where
        phi ias s = map (`psi` s) ias
        psi (i,_) r = (i,r)
