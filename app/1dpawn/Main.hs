...
solve :: Solver
solve = \ case
    (n,as,ls) -> case foldl phi (ns,am) ls of
        (_,bm)    -> IM.elems bm
        where
            ns = IS.fromList as
            am = IM.fromList (zip [1..] as)
            phi (ss,ms) i = case succ $ fromJust $ IM.lookup i ms of
                j -> bool (ss',ms') (ss,ms) (j > n || IS.member j ss)
                    where
                        ss' = IS.insert j $ IS.delete (pred j) ss
                        ms' = IM.update (Just . succ) i ms

wrap :: Solver -> ([[I]] -> [[O]])
wrap f = \ case
    (n:_):as:ls:_ -> case f (n,as,ls) of
        r -> [r]
